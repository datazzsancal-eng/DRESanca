...
const TemplateListPage: React.FC<TemplateListPageProps> = ({ onEditTemplate, onAddNew }) => {
  ...
  const handleViewTemplate = async (template: Template) => {
      if (!template.dre_cont) {
        setError("Este template não possui um código de controle para visualização.");
      ...
      try {
        const response = await fetch(`https://webhook.moondog-ia.tech/webhook/temp_dre?cntr=${template.dre_cont}`);
        if (!response.ok) {
    ...
    const handleExportPdf = () => {
        if (!selectedTemplate || filteredViewData.length === 0) return;

        const { jsPDF } = jspdf;
        const doc = new jsPDF();

        const title = `Template: ${selectedTemplate.dre_nome}`;
        ...
        doc.save(`template_${selectedTemplate.dre_nome?.toLowerCase().replace(/\s/g, '_')}.pdf`);
    };
              ...
              <tr key={template.id} className="hover:bg-gray-700/50">
                ...
                <td className="px-4 py-2 text-right whitespace-nowrap">
                  <div className="flex items-center justify-end space-x-4">
                     <button onClick={() => handleViewTemplate(template)} className="text-cyan-400 hover:text-cyan-300" aria-label="Visualizar">
